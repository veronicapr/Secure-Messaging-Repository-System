...
#* AES Cipher algorithm mode CBC*#
class AESCipher(object):

    def __init__(self, key):
        self.bs = 32
        self.key = hashlib.sha256(key.encode('hex')).digest()
...
class DiffieHellman:
    ...
    def genRandom(self, number_bits):
        """Secure random number generator"""
        rand = 0
        number_bytes = number_bits // 8 + 8
        while rand.bit_length() < number_bits:
            rand = int(binascii.hexlify(os.urandom(number_bytes)), base=16)
        return rand

    def genPrivateKey(self, number_bits):
        """Generate private key"""
        return self.genRandom(number_bits)

    def genPublicKey(self):
        """Generate public key X with g**x % p."""
        x = pow(self.generator, self.privateKey, self.prime)
        return x

    def genSecret(self, other_key):
        """Generate secret"""
        return pow(other_key, self.privateKey, self.prime)

    def genKey(self, other_key):
        """Derive the shared secret and hash said secret to obtain the shared key."""
        shared_secret = str(self.genSecret(other_key))

        s = hashlib.sha256()
        s.update(bytes(shared_secret))
        self.key = s.digest()

    def getKey(self):
        """ Function used for verification in client and server"""
        return self.key
