...
class Server:
    def __init__(self, host, port):
        ...
        self.ss.listen(10)
        log(logging.INFO, "Secure IM server listening on %s" % str(self.ss.getsockname()))
    ...
    def flushin(self, s):
        ...
        try:
            data = s.recv(BUFSIZE)
            log(logging.DEBUG, "Received data from %s. Message: %r" % (client, data))
        except:
    ...
    def flushout(self, s):
        ...
        try:
            ...
            log(logging.DEBUG, "Sent %d bytes to %s. Message: %r" % (sent, client, client.bufout[:sent]))
        ...
        while True:
            ...
            for s in rl:
                if s is self.ss:
                    ...
                    log(logging.ERROR, "Incoming, but %s not in clients anymore" % s)
            ...
            for s in wl:
                if s in self.clients:
                    ...
                    log(logging.ERROR, "Outgoing, but %s not in clients anymore" % s)
...
if __name__ == "__main__":
    ...
    logging.basicConfig(stream=sys.stdout, level=logging.DEBUG, formatter=logging.Formatter('%(asctime)s - %(levelname)s - %(message)s'))
